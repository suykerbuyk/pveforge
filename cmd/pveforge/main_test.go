...
var stderrSites = map[string]int{
	...
	"access.go: newACLGrantCmd: fmt.Fprintf(cmd.ErrOrStderr(), \"warning: %s: %s on %s, granted to %s, confers %s: its holder can widen its own or anyone's access\\n\", args[0], g.Grant.Role, g.Grant.Path, principals[0], strings.Join(g.Privs, \",\"))":                                        1,
	"vmsnapshot.go: newVMSnapshotRollbackCmd: witnessFailure(cmd.ErrOrStderr(), cmd, args[0], vmid, name, status, err)":                                                                                                                                                                            1,
	"vmsnapshot.go: witnessFailure: fmt.Fprintf(errOut, \"warning: %s: vm %d: witness output: %s\\n\", targetID, vmid, kvjson.QuoteValue(witnessExcerpt(status.OutData)))":                                                                                                                         1,
	"vm.go: newVMCreateCmd: fmt.Fprintf(cmd.ErrOrStderr(), \"warning: %s: vm %d: tag %s is not yet in PVE's cluster resource list after %s; a --unique-tag check made now may not see this VM\\n\", args[0], resolved, kvjson.QuoteValue(uniqueTag), tagVisibilityBound)":                          1,
	...
	"exec.go: newExecCmd: fmt.Fprintf(cmd.ErrOrStderr(), \"notice: exec: handing token %s of %s to %s\\n\", kvjson.QuoteValue(t.Token.ID), kvjson.QuoteValue(targetID), kvjson.QuoteValue(filepath.Base(path)))": 1,
	"main.go: realMain: runRoot(root, os.Stderr)":                                                                                 1,
	"main.go: bareGroup: w := cmd.ErrOrStderr()":                                                                                  1,
	"main.go: bareGroup: fmt.Fprintf(w, \"%s\\n\\n\", usage)":                                                                     1,
	"main.go: bareGroup: fmt.Fprint(w, cmd.UsageString())":                                                                        1,
	...
	"main.go: runRoot: taskWarningsNotice(stderr)":                                                                                1,
	"main.go: taskWarningsNotice: fmt.Fprintf(stderr, \"notice: PVE task %s on node %s succeeded with warnings (%s): its task log has them\\n\", kvjson.QuoteValue(boundErrText(upid)), kvjson.QuoteValue(boundErrText(node)), kvjson.QuoteValue(boundErrText(exitStatus)))": 1,
	"reread.go: warnNotReread: fmt.Fprintf(w, \"warning: %s: %s %s: the write was applied but its result could not be re-read: %s\\n\", target, kind, kvjson.QuoteValue(id), kvjson.QuoteValue(boundErrText(afterErr.Error())))":                                             1,
	"vm.go: newVMSetCmd: warnNotReread(cmd.ErrOrStderr(), args[0], \"vm\", strconv.Itoa(vmid), res.AfterErr)":                                                                                                                                                                1,
	"network.go: newNetworkBridgeCreateCmd: warnNotReread(cmd.ErrOrStderr(), args[0], \"bridge\", args[1], res.AfterErr)":                                                                                                                                                    1,
	"network.go: newNetworkBridgeDestroyCmd: warnNotReread(cmd.ErrOrStderr(), args[0], \"bridge\", args[1], res.AfterErr)":                                                                                                                                                   1,
	"network.go: newNetworkSetCmd: warnNotReread(cmd.ErrOrStderr(), args[0], \"interface\", args[1], res.AfterErr)":                                                                                                                                                          1,
	"access.go: newUserEnsureCmd: warnNotReread(cmd.ErrOrStderr(), targetID, \"user\", userID, res.AfterErr)":                                                                                                                                                                1,
	"access.go: newGroupEnsureCmd: warnNotReread(cmd.ErrOrStderr(), targetID, \"group\", groupID, res.AfterErr)":                                                                                                                                                             1,
	...
	"vm.go: newVMSetCmd: reportPending(cmd.ErrOrStderr(), args[0], vmid, op, res.Changed || op.Wrote(), res.PostApplyErr)":                                                                                                                                                   1,
	"vm.go: reportPending: warnPostCheck(errOut, targetID, \"vm\", strconv.Itoa(vmid), changed, question, postErr)":                                                                                                                                                          1,
	"vm.go: reportPending: fmt.Fprintf(errOut, \"notice: %s: vm %d: %s is already set but still pending: it takes effect at the VM's next cold boot\\n\", targetID, vmid, kvjson.QuoteKey(f))":                                                                               1,
	"vm.go: reportPending: fmt.Fprintf(errOut, \"notice: %s: vm %d: delete=%s is already done but still pending: it takes effect at the VM's next cold boot\\n\", targetID, vmid, kvjson.QuoteValue(f))":                                                                     1,
	"vm.go: reportPending: fmt.Fprintf(errOut, \"notice: %s: vm %d: %s is already set but not yet on the cloud-init drive: the guest sees it after the drive is regenerated at the VM's next start\\n\", targetID, vmid, kvjson.QuoteKey(f))":                                1,
	"vm.go: reportPending: fmt.Fprintf(errOut, \"notice: %s: vm %d: delete=%s is already done but not yet on the cloud-init drive: the guest sees it after the drive is regenerated at the VM's next start\\n\", targetID, vmid, kvjson.QuoteValue(f))":                      1,
	"vm.go: newVMCreateCmd: warnNotReread(cmd.ErrOrStderr(), args[0], \"vm\", strconv.Itoa(resolved), res.AfterErr)":                                                                                                                                                         1,
	"vm.go: newVMCreateCmd: warnPostCheck(cmd.ErrOrStderr(), args[0], \"vm\", strconv.Itoa(resolved), res.Changed, \"PVE then reported it does not exist\", res.PostApplyErr)":                                                                                               1,
	"access.go: newUserEnsureCmd: warnPostCheck(cmd.ErrOrStderr(), targetID, \"user\", userID, res.Changed, \"it does not read back as requested\", res.PostApplyErr)":                                                                                                       1,
	"access.go: newGroupEnsureCmd: warnPostCheck(cmd.ErrOrStderr(), targetID, \"group\", groupID, res.Changed, \"it does not read back as requested\", res.PostApplyErr)":                                                                                                    1,
	"reread.go: warnPostCheck: fmt.Fprintf(w, \"warning: %s: %s %s: %s %s: %s\\n\", target, kind, kvjson.QuoteValue(id), lead, question, kvjson.QuoteValue(boundErrText(postErr.Error())))":                                                                                  1,
}
