...
func TestRunRootSuccess_DiscoverDevice(t *testing.T) {
	pinRunRoot(t, "description=An emulated NVMe drive attached via the args: raw-QEMU escape hatch (Proxmox has no first-class NVMe bus type).\nproperties={\"backing\":{\"type\":\"string\",\"description\":\"The QEMU -drive file= target: an absolute host path (e.g. \\\"/dev/pve/vm-100-disk-1\\\"), not a PVE volid or a URL. QEMU opens it as root on the host, so the path is a trust decision for whoever configures it: pveforge refuses only a malformed path, not a sensitive one.\",\"pattern\":\"^(/[A-Za-z0-9_.-]*[A-Za-z0-9_-][A-Za-z0-9_.-]*)+$\"},\"format\":{\"type\":\"string\",\"description\":\"The QEMU -drive format= value (e.g. \\\"raw\\\", \\\"qcow2\\\"). PVE/QEMU infer it when unset.\",\"pattern\":\"^[A-Za-z0-9]+$\",\"optional\":true},\"serial\":{\"type\":\"string\",\"description\":\"The drive's emulated serial number, surfaced inside the guest OS.\",\"pattern\":\"^[A-Za-z0-9\\\\-_]+$\"}}\nrequired=[\"serial\",\"backing\"]\ntype=object\n", "discover", "device", "NVMeDrive")
}
