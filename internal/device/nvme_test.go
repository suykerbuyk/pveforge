...
func TestNVMeDrive_Validate(t *testing.T) {
	cases := []struct {
		...
		{"valid, no format", NVMeDrive{Serial: "SN123", Backing: "local-lvm:vm-100-disk-1"}, false},
		{"valid, with format", NVMeDrive{Serial: "SN-123_x", Backing: "/var/lib/vz/images/100/disk.raw", Format: "raw"}, false},
		{"missing serial", NVMeDrive{Backing: "local-lvm:vm-100-disk-1"}, true},
		{"missing backing", NVMeDrive{Serial: "SN123"}, true},
		{"serial with comma", NVMeDrive{Serial: "SN,123", Backing: "local-lvm:vm-100-disk-1"}, true},
		{"serial with space", NVMeDrive{Serial: "SN 123", Backing: "local-lvm:vm-100-disk-1"}, true},
		{"serial with quote", NVMeDrive{Serial: `SN"123`, Backing: "local-lvm:vm-100-disk-1"}, true},
		{"backing with comma", NVMeDrive{Serial: "SN123", Backing: "local-lvm:vm-100,disk-1"}, true},
		{"backing with space", NVMeDrive{Serial: "SN123", Backing: "local lvm:vm-100-disk-1"}, true},
		{"format with comma", NVMeDrive{Serial: "SN123", Backing: "local-lvm:vm-100-disk-1", Format: "raw,evil=1"}, true},
	}
	for _, c := range cases {
...
func TestNVMeDrive_Apply_FragmentAndMerge(t *testing.T) {
	...
	drive := NVMeDrive{Serial: "SN123", Backing: "local-lvm:vm-100-disk-1", Format: "raw"}
	...
	want := "-cpu host -device nvme,drive=nvme-SN123,serial=SN123 -drive file=local-lvm:vm-100-disk-1,if=none,id=nvme-SN123,format=raw"
...
func TestNVMeDrive_Apply_NoFormat(t *testing.T) {
	...
	drive := NVMeDrive{Serial: "SN123", Backing: "local-lvm:vm-100-disk-1"}

	if err := drive.Apply(context.Background(), client, 100); err != nil {
		t.Fatalf("Apply: %v", err)
	}

	want := "-device nvme,drive=nvme-SN123,serial=SN123 -drive file=local-lvm:vm-100-disk-1,if=none,id=nvme-SN123"
	if client.lastValue != want {
...
func TestNVMeDrive_Apply_InvalidDrive(t *testing.T) {
	client := &fakeClient{node: "qa-pve-01"}
	drive := NVMeDrive{Serial: "", Backing: "local-lvm:vm-100-disk-1"}
