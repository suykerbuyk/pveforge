...
type InstallResult struct {
	// HostKeyFingerprint is the target's SSH host key fingerprint,
	// captured trust-on-first-use during this connection. The caller must
	// persist this and use PinnedHostKeyCallback for every later
...
func InstallPubkeyViaPassword(ctx context.Context, addr, user, password, authorizedKeyLine string) (*InstallResult, error) {
	var captured CapturedHostKey
	client, err := DialWithPassword(ctx, addr, user, password, CaptureHostKeyCallback(&captured))
	if err != nil {
